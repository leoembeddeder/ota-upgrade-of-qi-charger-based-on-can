# -*- coding: utf-8 -*-
"""
ZCANPRO 扩展脚本 — 从 APP 升级 Slot A

强制入口 APP：10 02 → 27 → 11 01 进 Boot，再写入 Slot A。
MCU 必须正在跑 APP（通常是 Slot B）。若已在 Boot，请用 from_boot 脚本。

导入: 高级功能 -> 扩展脚本 -> 打开本文件
运行前: 先打开 CAN 通道 (250 kbps, Classical CAN, 扩展帧)
需要: Python 3.8 32 位（ZCANPRO 扩展脚本要求）
固件: app bin/app_slot_a.bin（由 pack_image_slotA_1_1_1.py 生成）
"""
...
# ======== 用户配置 ========
# Slot A：Keil Target IROM1 = 0x08004100；Slot B：IROM1 = 0x08010100。不用 scatter。
...
def z_main():
    global stopTask
    stopTask = False
    _log("======== Qi CAN-UDS OTA Slot A (入口=%s) ========" % ENTRY_MODE)
