...
流程：
  0. 运行侧探测（唤醒收敛 + 正证据判定，仅 APP 可写 SN；Boot safe mode / 无应答退出）
  1. 进入编程会话 (0x10 0x02)
...
def _wdbi_with_s3_guard(bus_id, payload, priv, wait_pending_s=0):
    ...
    重发本脚本会话控制 10 02 + 既有 send_security_key 完整重解锁 + 重试写一次。

    固件门禁（can_protocol.c:1050-1062）：写 0xF18C DID_SERIAL_NUMBER 要求
    SESSION_PROGRAMMING+security_unlocked——NRC 0x22=会话不满足（S3 超时回
    default 的典型表现），0x33=安全态被清。SN 长度>32 回 0x31、NVM 写失败
    回 0x72（:1090-1115），不会误入本兜底。会话字节与本脚本业务链一致
    （10 02 编程会话；A 项不改 sn_write，门禁匹配已由 evaluator 核实）。
    日志注明「S3 超时恢复」。
    """
    try:
        ...
        _log("2E 写 NRC 0x%02X：疑似 S3 会话超时（会话回 default / 安全态被清）——"
             "S3 超时恢复：重发 10 02 编程会话 + 完整重解锁 + 重试写一次" % e.nrc)
...
# 与 zcanpro_charge_start.py probe_location 同构（固件依据文件:行号见提交
# 报告）：绝不按 NRC 值分叉定位——0x34/22 2113 的任何应答（正响应或任意
# NRC，且非 0xFE 标记）= 在 APP；22 2113 应答中 62 21 13 后字节为 0xFE
# = Boot safe mode（APP 的 DID 0x2113 也正响应，slot 字节 0x00/0x01 永不
# 0xFE）；无应答 ≠ 不在 APP，先 Standby 唤醒收敛（3E 80 burst）+ 释放
# UDS 通道监听 0x18FF260D 生命周期帧再判。
...
def run_sn_write(bus_id, sn_code):
    ...
    _log("写入数据: " + _hex(sn32))
    # 经 S3 兜底包装：NRC 0x22/0x33（会话回 default/安全态被清）时重发
    # 10 02 编程会话 + 完整重解锁 + 重试写一次（wait_pending_s=10 覆盖 NVM 延迟）
    _wdbi_with_s3_guard(bus_id, [0xF1, 0x8C] + sn32, priv, wait_pending_s=10)
