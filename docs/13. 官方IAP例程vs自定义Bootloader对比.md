...
| 维度 | 官方 IAP | 自定义 Bootloader |
|------|----------|-------------------|
| APP→Boot 触发 | APP 收到 `0x5A 0xA5` → 写 `0x41544B38` 到 flag 区（`APP_START_ADDR - 0x800`） → NVIC 复位 | 下载在 APP 内完成（`0x31/0x34/0x36/0x37`），`0x37` 验签+commit_backup 后 APP 自动复位（`11 01` ECUReset 服务已删除 2026-09-23） |
...
| OTA 触发 | APP 收到 `0x5AA5` → 写 flag → NVIC 复位 | APP 内 `0x37` 验签+commit_backup 后自动复位（`11 01` 已删除 2026-09-23） |
