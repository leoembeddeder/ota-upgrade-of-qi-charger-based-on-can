...
> 更新：2026-09-17（对齐 OTA 架构反转：下载迁入 APP，Boot 只选槽+验签+跳转）
...
| 维度 | 官方 IAP | 自定义 Bootloader |
|------|----------|-------------------|
| APP→Boot 触发 | APP 收到 `0x5A 0xA5` → 写 `0x41544B38` 到 flag 区（`APP_START_ADDR - 0x800`） → NVIC 复位 | 下载在 APP 内完成（`0x31/0x34/0x36/0x37`），`0x37` 验签通过后 APP 写 metadata `trial_state=PENDING`；`0x11 0x01` 仅复位（`handle_ecu_reset` 不写 metadata） |
| Boot 入口判断 | 读 flag 区，`== 0x41544B38` 则进升级流程 | 读 metadata，按 `trial_state`/`active_slot` 选槽；`ota_state=DOWNLOADING` 已忽略（legacy，`main.c:70`） |
...
| APP 侧配合 | APP 需实现 `iap_command_handle()` 监听 `0x5AA5` 并写 flag | APP 自己完成擦槽+下载+验签+写 trial metadata（`ota_download.c`），Boot 只选槽跳转 |
| 主机侧工具 | PC 串口工具发 `0x5A 0x01` 启动升级 | ZCANPRO `zcanpro_ext_ota_auto.py`（APP 内全量擦写下载） |
...
|------|----------|-------------------|
| Boot 区 | `0x08000000 ~ 0x08003FFF`（16KB） | `0x08000000 ~ 0x08003FFF`（16KB，只选槽+验签+跳转，无 UDS） |
| 升级 flag | `0x08003800`（Boot 前一个扇区，2KB） | metadata `0x0801C000` + backup `0x0801C800`（各 2KB） |
| APP 区 | 单槽 `0x08004000` 起（全部剩余空间） | 双槽：A=`0x08004000`（48KB）、B=`0x08010000`（48KB） |
| APP 入口 | `0x08004000` | Slot A: `0x08004100`（+256B header）、Slot B: `0x08010100` |
| NVM 配置 | 无 | `0x0801E000 ~ 0x0801FFFF`（8KB） |
| Device Info | 无 | `0x0801D000`（4KB，SN 等） |
| 镜像头 | 无 | 256B XATO 头（magic + length + CRC32 + ECDSA 签名 + 时间戳；0x4C 为保留占位区，原 version 字段声明已删除，固定填 `0x00`，不携带版本号） |
...
| Safe Mode | ❌ 无（验证失败死循环） | ✅ 验证失败进 safe mode（挂起，靠产线镜像救砖） |
...
| OTA 触发 | APP 收到 `0x5AA5` → 写 flag → NVIC 复位 | APP 内 `0x37` 验签通过 → 写 trial PENDING → `0x11 0x01` 仅复位 |
| APP 验证 | Boot 不验证 APP 内容 | Boot 对 APP 做完整验签（magic + length + CRC + SHA-256 + ECDSA） |
| 版本管理 | 无 | DID `0xF195`（32B ASCII，取 APP 编译常量 SW_VERSION_STR；不读镜像头 / OTA metadata） |
...
| 维度 | 官方 IAP | 自定义 Bootloader |
|------|----------|-------------------|
| Boot 源文件 | 7 个（iap/flash/usart/tmr/main/clock/int） | 12 个（boot_verify/boot_jump/boot_metadata/boot_trial/boot_safe_mode + can_driver/isotp/sit1145/sha256/uECC/timer_drv/main） |
| Boot 编译大小 | ~4KB（预估） | ~11.3KB（`bootloader.bin` 实测 11576B） |
| 核心逻辑复杂度 | 低（`iap.c` 状态机 ~200 行） | 中（Boot 只选槽+验签+跳转，`boot_safe_mode.c` 仅 19 行挂起；UDS/擦写/下载全在 APP `ota_download.c`） |
...
自定义:    16KB Boot（只选槽+验签+跳转）+ 双槽 A/B + APP 内 UDS 下载 + ECDSA 签名 + trial boot + 自动回滚
```
